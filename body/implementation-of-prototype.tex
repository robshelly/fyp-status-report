...
In order to asses the technical feasibility of this project three \textit{Proof of Concepts} \added[id=RB]{(POCs)}have been defined.
...
\end{POC}
	...
	\subsubsection{Issues Encountered}
	As GPG keys are managed by keyrings they cannot be stored in Jenkins using the Credentials plugin nor can they be used to decrypt remote files. For this reason, the GPG key was imported into a keyring on the \textit{restoration} server. This enable\added[id=RB]{d} \deleted[id=RB]{to} successful decryption of the file.
	...
	\end{itemize}
	\note[id=RB]{This section is very good; concisely sums up the work done so far and the consistent layout for each POC makes it flow well.}
